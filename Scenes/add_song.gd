...
@onready var playlist_options : OptionButton = $AddSong/PlaylistRow/PlaylistOptions
var upload_path = ""
func set_path(path):
	upload_path = path
...
func set_playlists():
	var playlist_dict = PlaylistPlayer.playlist_dictionary
	for id in playlist_dict.keys():
		var playlist = playlist_dict[id]
		playlist_options.add_item(playlist.name, id)
...
func add_song():
	...
	var playlist_id = playlist_options.selected
	if (playlist_id != -1):
